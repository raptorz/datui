...
function friendship_exists($user_a) {
  $request = DABR_API.'friendships/show.json?target_screen_name=' . $user_a;
...
function twitter_block_exists($query) 
{
	...
	$request = DABR_API.'blocks/blocking/ids.json';
...
function twitter_trends_page($query) 
{
  ...
  $request = DABR_APIS.'trends/' . $trend_type . '.json';
...
function twitter_twitpic_mail($media, $message, $user, $pass) {
    $to_email = $user . '.' . $pass .'@twitpic.com';
    $message = isset($message) ? $message : '';
    $subject = "=?UTF-8?B?" . base64_encode($message) . "?=";
    $fileName = $media['name'];
    $picname  = $media['tmp_name'];
    $mimeType = $media['type'] ? $media['type'] : "application/unknown";

    $boundary = uniqid("");
    $headers = "Content-type: multipart/mixed; boundary= $boundary\r\n";
    $headers .= "From:".TWITPIC_MAIL_SENDER."\r\n";
    // 打开文件
    $fp = fopen($picname, "rb");
    // 把整个文件读入一个变量
    $read = fread($fp, filesize($picname));
    //我们用base64方法把它编码
...
--$boundary--";
    //return $to_email ."<br/>" . $headers . "<br/>" . $subject . "<br/>" . $body;
    //发送邮件
    if(mail($to_email,$subject,$body,$headers))
        return "<mediaid>-1</mediaid>";
    else
        return "fail!";
}
...
function twitter_twitpic_page($query) {
  if ($_POST['message']) {
    if (user_type() == 'oauth') {
        $response = twitter_twitpic_mail($_FILES['media'],
    ...
    if (preg_match('#mediaid>(.*)</mediaid#', $response, $matches)) {
      ...
      if ($query[2]!='-1')
          $content .= "<p><img src='http://twitpic.com/show/thumb/{$query[2]}' alt='' /></p>";
  } elseif ($query[1] == 'fail') {
    $content = '<p>Twitpic upload failed. No idea why!</p>';
  } else {
      $content = '<form method="post" action="twitpic" enctype="multipart/form-data">Image <input type="file" name="media" /><br />Message: <input type="text" name="message" maxlength="120" /><br />';
      if (user_type() == 'oauth') {
          $content .= 'your twitpic mail:</td><td><input type="text" name="tp_user" size=10 />.<input type="password" name="tp_pass" size=4 />@twitpic.com<br/>';
      }
...
function twitter_status_page($query) {
  $id = (string) $query[1];
  if (is_numeric($id)) {
    $request = DABR_API."statuses/show/{$id}.json";
...
function twitter_thread_timeline($thread_id) {
  $request = DABR_APIS."search/thread/{$thread_id}";
...
function twitter_delete_page($query) {
  ...
  $id = (string) $query[1];
  if (is_numeric($id)) {
    $request = DABR_API."statuses/destroy/{$id}.json?page=".intval($_GET['page']);
...
function twitter_follow_page($query) {
  ...
  if ($user) {
    if($query[0] == 'follow'){
      $request = DABR_API."friendships/create/{$user}.json";
    } else {
      $request = DABR_API."friendships/destroy/{$user}.json";
    }
...
function twitter_block_page($query) {
  ...
  if ($user) {
    if($query[0] == 'block'){
      $request = DABR_API."blocks/create/{$user}.json";
    } else {
      $request = DABR_API."blocks/destroy/{$user}.json";
    }
    twitter_process($request, true);
...
function twitter_spam_page($query) 
{
	...
	$request = DABR_API."report_spam.json";
...
function twitter_friends_page($query) {
  ...
  $request = DABR_API."statuses/friends/{$user}.xml";
...
function twitter_followers_page($query) {
  ...
  $request = DABR_API."statuses/followers/{$user}.xml";
  $tl = lists_paginated_process($request);
  $content = theme('followers', $tl);
...
function twitter_update() {
  ...
  if ($status) {
    $request = DABR_API.'statuses/update.json';
...
function twitter_retweet($query) {
  ...
  $id = $query[1];
  if (is_numeric($id)) {
    $request = DABR_API.'statuses/retweet/'.$id.'.xml';
    twitter_process($request, true);
  }
  twitter_refresh($_POST['from'] ? $_POST['from'] : '');
}
...
function twitter_public_page() {
  $request = DABR_API.'statuses/public_timeline.json?page='.intval($_GET['page']);
...
function twitter_replies_page() {
  $request = DABR_API.'statuses/replies.json?page='.intval($_GET['page']);
  $tl = twitter_process($request);
...
function twitter_directs_page($query) {
  ...
  switch ($action) {
    case 'delete':
      ...
      $request = DABR_API."direct_messages/destroy/$id.json";
      twitter_process($request, true);
      twitter_refresh();
    ...
    case 'send':
      ...
      $request = DABR_API.'direct_messages/new.json';
      twitter_process($request, array('user' => $to, 'text' => $message));
      twitter_refresh('directs/sent');
    
    case 'sent':
      $request = DABR_API.'direct_messages/sent.json?page='.intval($_GET['page']);
    ...
    case 'inbox':
    default:
      $request = DABR_API.'direct_messages.json?page='.intval($_GET['page']);
...
function twitter_search($search_query) {
  $page = (int) $_GET['page'];
  if ($page == 0) $page = 1;
  $request = DABR_APIS.'search.json?q=' . urlencode($search_query).'&page='.$page;
  $tl = twitter_process($request);
...
function twitter_user_page($query) {
  ...
  if ($screen_name) {
    ...
    if (isset($user->status)) {
      $request = DABR_API."statuses/user_timeline.json?screen_name={$screen_name}&page=".intval($_GET['page']);
      $tl = twitter_process($request);
...
function twitter_favourites_page($query) {
  ...
  $request = DABR_API."favorites/{$screen_name}.json?page=".intval($_GET['page']);
  $tl = twitter_process($request);
...
function twitter_mark_favourite_page($query) {
  ...
  if ($query[0] == 'unfavourite') {
    $request = DABR_API."favorites/destroy/$id.json";
  } else {
    $request = DABR_API."favorites/create/$id.json";
  }
  twitter_process($request, true);
  twitter_refresh();
}
...
function twitter_home_page() {
  user_ensure_authenticated();
  $request = DABR_API.'statuses/home_timeline.json?count=20&page='.intval($_GET['page']);
  $tl = twitter_process($request);
...
function theme_retweet($status) {
  ...
  $content .= js_counter("status");  
    ...
    $content.="<br />Or Twitter's new style retweet<br /><form action='twitter-retweet/{$status->id_str}' method='post'><input type='hidden' name='from' value='$from' /><input type='submit' value='Twitter Retweet'></form>";
  }
  return $content;
}
...
function theme_status_time_link($status, $is_link = true) {
  ...
  if ($time > 0) {
    if (twitter_date('dmy') == twitter_date('dmy', $time)) {
      $out = format_interval(time() + TIME_DIFF * 3600 - $time, 1). ' ago';
    } else {
  ...
  if ($is_link)
    $out = "<a href='status/{$status->id_str}'>$out</a>";
...
function twitter_date($format, $timestamp = null) {
  ...
  if (!isset($offset)) {
        ...
        $offset = strtotime($now.' '.TIME_ZONE_DIFF.' hour') - time();
/*    if (user_is_authenticated()) {
...
function twitter_user_info($username = null) {
  if (!$username)
  $username = user_current_username();
  $request = DABR_API."users/show.json?screen_name=$username";
...
function theme_action_icons($status) {
  ...
  if (!$status->is_direct) {
    $actions[] = theme('action_icon', "user/{$from}/reply/{$status->id_str}", 'images/reply.png', '@');
  }
  ...
  if (!$status->is_direct) {
    if ($status->favorited == '1') {
      $actions[] = theme('action_icon', "unfavourite/{$status->id_str}", 'images/star.png', 'UNFAV');
    } else {
      $actions[] = theme('action_icon', "favourite/{$status->id_str}", 'images/star_grey.png', 'FAV');
    }
    $actions[] = theme('action_icon', "retweet/{$status->id_str}", 'images/retweet.png', 'RT');
    if (user_is_current_user($from)) {
      $actions[] = theme('action_icon', "confirm/delete/{$status->id_str}", 'images/trash.gif', 'DEL');
    }
  } else {
    $actions[] = theme('action_icon', "directs/delete/{$status->id_str}", 'images/trash.gif', 'DEL');
  }
