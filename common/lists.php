...
function twitter_lists_tweets($user, $list) {
	...
	$url = DABR_API."{$user}/lists/{$list}/statuses.json";
...
function twitter_lists_user_lists($user) {
	...
	return lists_paginated_process(DABR_API."{$user}/lists.xml");
}
...
function twitter_lists_list_subscribers($user, $list) {
	...
	return lists_paginated_process(DABR_API."{$user}/{$list}/subscribers.xml");
}
